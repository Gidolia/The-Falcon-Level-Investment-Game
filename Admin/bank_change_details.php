...
<head>
  ...
  <title>Customer Change Bank Details </title>
  ...
  <div class="container-fluid content-top-gap">
    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb my-breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Customer Change Bank Details List</li>
      </ol>
    </nav>
            ...
            <div class="card card_border col-md-12">
                ...
                <div class="card-box table-responsive">
                     <table id="datatable" class="table table-striped table-bordered">
                      ...
                      <tbody>
                          <?php 
                                ...
                                $e="SELECT * FROM `bank_kyc` WHERE `s_status`='p'";
                                $d=$con->query($e);
                     ...
                     if(isset($_POST[clear_submit]))
                     {
                       ...
                       $sql="UPDATE `bank_kyc` SET `s_status` = 'c' WHERE `bank_kyc`.`bnk_id` = '$_POST[bnk_id]';";
                        $ssl="UPDATE `bank_kyc` SET `status` = 'y' WHERE `bank_kyc`.`bnk_id` = '$_POST[bnk_id]';";
                       //echo $sql;
                        if ($con->query($sql) === TRUE && $con->query($ssl) === TRUE) {
                          echo "<script>alert('Change Bank Details Approved successfully');
                          location.href='bank_change_details.php'</script>";
                        } else {
                     ...
                     if(isset($_POST[reject_submit]))
                     {
                       ...
                       $sql="UPDATE `bank_kyc` SET `s_status` = 'n' WHERE `bank_kyc`.`bnk_id` = '$_POST[bnk_id]';";
                     
                       $sl="UPDATE `bank_kyc` SET `status` = 'n' WHERE `bank_kyc`.`bnk_id` = '$_POST[bnk_id]';"; 
                       
                        if ($con->query($sql) === TRUE && $con->query($sl) === TRUE) {
                          echo "<script>alert('Change Bank Details reject successfully');
